...
Class DistributiveLattice (A : Type) `{Lattice A} := {
  distrib : forall (a b c : A), a ⊓ (b ⊔ c) = (a ⊓ b) ⊔ (a ⊓ c)
}.

Reserved Notation "¬ a" (at level 70, right associativity).
Class BooleanAlgebra (A : Type) `{DistributiveLattice A} := {
  negate : A -> A where "¬ a" := (negate a);

  negate_top: forall a, a ⊓ (¬ a) = bot;
  negate_bot: forall a, a ⊔ (¬ a) = top;
}.

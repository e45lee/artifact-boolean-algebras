...
Require Export Fsub.Base.LabelMap.
Require Export Fsub.Base.Label.
Require Export Fsub.FbMeta.Metatheory.
Require Export String.
Require Export Coq.ZArith.ZArith.
Require Export Fsub.Base.Lattice.
...
Inductive typing : env -> exp -> qtyp -> Prop :=
  | typing_int : forall E P i,
      wf_env E ->
      wf_qua E P ->
      typing E (exp_int P i) (qtyp_qtyp P typ_int)
  ...
  | typing_if0 : forall E Q e1 e2 e3 T,
      typing E e1 (qtyp_qtyp Q typ_int) ->
      typing E e2 T ->
      typing E e3 T ->
      typing E (exp_if0 e1 e2 e3) T
.
...
Inductive red : exp -> exp -> Prop :=
  ...
  | red_upqual_int : forall P Q cP cQ i,
      qual P ->
      qual Q ->
      concretize P = Some cP ->
      concretize Q = Some cQ ->
      red (exp_upqual P (exp_int Q i)) (exp_int P i)
  ...
  | red_check_int : forall P Q cP cQ i,
      qual P ->
      qual Q ->
      concretize P = Some cP ->
      concretize Q = Some cQ ->
      cQ ≤ cP ->
      red (exp_check P (exp_int Q i)) (exp_int Q i)
  ...
  | red_check_qabs : forall P Q cP cQ V e1,
   ...
   | red_if0_test : forall e1 e1' e2 e3,
      red e1 e1' ->
      expr e2 ->
      expr e3 ->
      red (exp_if0 e1 e2 e3) (exp_if0 e1' e2 e3)
   | red_if0_true : forall Q e2 e3,
      qual Q ->
      expr e2 ->
      expr e3 ->
      red (exp_if0 (exp_int Q 0) e2 e3) e2
   | red_if0_false : forall Q e2 e3 (i : Z),
      qual Q ->
      expr e2 ->
      expr e3 ->
      (not (Z.eq i 0)) ->
      red (exp_if0 (exp_int Q i) e2 e3) e3
.
...
#[export] Hint Resolve subqual_top subqual_bot subqual_refl_qvar subqual_trans_qvar 
                       ...
                       subqual_refl_negate 
                       subqual_lift_elem subqual_eval_elim subqual_eval_intro subqual_lift_elem : core.
#[export] Hint Resolve sub_top sub_refl_int sub_refl_tvar sub_arrow : core.
#[export] Hint Resolve sub_sum sub_pair : core.
#[export] Hint Resolve typing_int typing_var typing_app typing_tapp typing_sub typing_qapp : core.

...
Proof with simpl_env; eauto using 
  ...
  all: try rename select (typing_ctx _ _ _ T _) into TypCtx.
  all: try rename select (typing_ctx _ _ _ T0 _) into TypCtx.
  ...
  Case "step_check".
    ...
    SCase "step_check_abs".
      inversion select (state_step _ _ = state_step _ _); subst...
      dependent induction TypCtx; subst...
      unshelve epose proof (typing_ctx_regular _ _ _ _ _ TypCtx) as [WfE [WfQT WfQua]]...
      eapply typing_step with (Q := Q) (T := qtyp_qtyp P0 T)...
      eapply typing_sub with (S := qtyp_qtyp (qua_meet P0 P) T)...
  ...
  Case "step_without".
    ...
    rename select (concretize _ = _) into EqT.
      rewrite EqT...
...
Lemma compatible_barriers : forall Q t Qk k T U,
  concretize Q = Some t ->
  subqual empty Q Qk ->
  typing_ctx empty Qk k T U ->
...
Lemma progress_value_step : forall v k U,
  value v ->
  typing_state empty (state_step v k) U ->
  done_state (state_step v k) \/ exists s', (state_step v k) --> s'.
Proof with eauto.
  intros * Val Typ.
  inversion Typ; subst.
  rename select (typing_ctx _ _ _ _ _) into TypCtx.
  ...
  Case "step_app".
    ...
    unshelve epose proof (compatible_barriers _ _ _ _ _ _ ComputeQf _ TypCtx)...
  Case "step_tapp".
  ...
  Case "step_upqual".
    inversion Val; subst...
    SCase "val_int".
      unshelve epose proof (typing_ctx_regular _ _ _ _ _ TypCtx)
        as [_ [WfTyp2 WfQua2]]...
...
Lemma progress : forall s U,
  typing_state empty s U ->
...
Proof with eauto.
  ...
  all: try rename select (typing _ _ _ _) into Typ.
  all: try rename select (typing_ctx _ _ _ _ _) into TypCtx.
  all: try rename select (typing_ctx _ _ _ _ _) into TypCtx.
  ...
  Case "typing_do".
    ...
    unshelve epose proof
      (compatible_barriers (qua_base b) b (qua_base b) c _ U _ _ TypCtx)...
    eapply subqual_reflexivity...
Qed.

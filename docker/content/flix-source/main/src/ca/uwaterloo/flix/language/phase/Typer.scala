...
object Typer {
  ...
  /**
    ...
    */
  private def shouldSubeffect(declSym: Symbol, body: KindedAst.Expr, eff: Type, subeffecting: Subeffecting)(implicit flix: Flix): Boolean = {
    ...
    if (enabled && !useless) subeffecting match {
      case Subeffecting.ModDefs => Summary.modDefSubEffVarsTracker.compute(declSym, (_, i) => i + 1)
      case Subeffecting.InsDefs => Summary.insDefSubEffVarsTracker.compute(declSym, (_, i) => i + 1)
